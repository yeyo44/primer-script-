...
difficulty=$1
os=$2
check="$(cat bundle.js | grep "so: \"$os\"" -C3 | grep "dificultad: \"$difficulty\"" -C4 | grep "name: " | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column)"

if [ "$check" ]; then
	echo -e "\n${purpleColour}[+]${endColour}${grayColour} Mostrando las máquinas cuyo sistema operativo es${endColour}${blueColour} $os${endColour}${grayColour} y la dificultad${endColour}${blueColour} $difficulty${endColour}"
	echo -e "${grayColour}$check${endColour}\n"
else
	echo -e "\n${grayColour}Los parámetros escritos son incorrectos.${endColour}\n"
fi
